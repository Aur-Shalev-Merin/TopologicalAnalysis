...
addprocs(12) # add, say, 12 processes
...
function generate_points_within_convex_hull(simplices, positions, volumes, N)
    cumulative_volumes = cumsum(volumes) / sum(volumes)
    random_points = Vector{Vector{Float64}}(undef, N)  # Initialize as a vector of vectors
    num_tetrahedra = size(simplices, 1)
    for i in 1:N
        # Randomly select a tetrahedron weighted by its volume
        r = rand()
        tet_index = searchsortedfirst(cumulative_volumes, r)
...
random_points = get_random_points_convex(random_csv_path)
...
# Flatten the array of arrays for plotting
...
ylabel!("Y")
zlabel!("Z")
title!("Random Points Within Convex Hull")

display(scatter3d(positions_matrix[:, 1], positions_matrix[:, 2], positions_matrix[:, 3], label="Original Points", markersize=2))
display(scatter3d!(random_points_matrix[:, 1], random_points_matrix[:, 2], random_points_matrix[:, 3], label="Random Points", markersize=1, alpha=0.5))
